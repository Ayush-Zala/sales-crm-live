...
class KonarchAccount extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'app:konarch-account';
    ...
    public function handle()
    {
        ...
        // die();
        $filePath = public_path('uploads/phone1.xlsx'); // Adjust the path as needed
        $string = "";
        ...
        foreach ($data[0] as $key => $row) {
            if ($key != 0) {
                ...
                // if ($row[15] != "") {
                //     $clientid = end($clientidarr);
                //     if (strpos($row[15], ';') !== false) {

                //         $clientemailarr = explode(";", $row[15]);
                //         $clientemailtypearr = explode(";", $row[16]);

                //         foreach ($clientemailarr as $key => $value) {
                //             $clientemailcount = ClientsEmail::where('mail', $value)->get();
                //             if (count($clientemailcount) < 1) {
                //                 $clientemail = new ClientsEmail();
                //                 $clientemail->clients_id = $clientid;
                //                 $clientemail->mail = $value;
                //                 $clientemail->type = $clientemailtypearr[$key];
                //                 $clientemail->created_at = carbon::now();
                //                 $clientemail->updated_at = carbon::now();
                //                 $clientemail->save();
                //             }
                //         }
                //     } else {
                //         $clientemailcount = ClientsEmail::where('mail', $row[15])->get();
                //         if (count($clientemailcount) < 1) {

                //             $clientemail = new ClientsEmail();
                //             $clientemail->clients_id = $clientid;
                //             $clientemail->mail = $row[15];
                //             $clientemail->type = $row[16];
                //             $clientemail->created_at = carbon::now();
                //             $clientemail->updated_at = carbon::now();
                //             $clientemail->save();
                //         }
