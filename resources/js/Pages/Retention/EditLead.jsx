...
export default function EditLead({ auth, details }) {
    ...
    const submit = (data) => {
        ...
        fetch(route("retention.update"), {
            ...
            .catch((error) => {
                toast.error("Error in updating retention");
            });
    };

    return (
        <AuthenticatedLayout user={auth.user}>
            <Head title="Edit Retention" />
            <MainContentTemplate
                title="Edit Retention"
                subtitle="Edit Retention from here"
                button="Go back"
                href={route("retention.index")}
                secondButton="View"
                secondButtonHref={route("retention.view", leadInfo[0]?.id)}
            >
                <Grid item xs={12}>
                    <FormContainer
